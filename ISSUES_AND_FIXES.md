...
| Date | Scope | Result |
|---|---|---|
| 2026-08-23 | Real multi-document upload (40 documents) through the live deployed service, retried after fixes | Pass. Found and fixed two real concurrency bugs along the way - see Resolved #18 below. |
...
### 18. Two real concurrency bugs in `consumer`: unsafe cross-thread RabbitMQ acks, unsynchronized embedding-model lazy-load
**Found:** 2026-08-23, processing many real documents through the live deployed service in quick succession
**Fixed:** 2026-08-23, `src/services/consumer/app.py`, `src/rag/store.py`

**Problem 1 — RabbitMQ ack/nack from worker threads:** `process_message()` runs on a `ThreadPoolExecutor` worker thread and called `channel.basic_ack()`/`basic_nack()` directly at the end, guarded only by a plain `threading.Lock()`. `pika`'s `BlockingConnection`/channel is not safe to touch from any thread other than the one running `channel.start_consuming()`'s I/O loop — a Python-level lock does not fix this, since the corruption isn't a Python race, it's `pika`'s internal frame/state handling assuming single-threaded access. Symptoms: real `"Channel is closed"` and `"IndexError('pop from an empty deque')"` errors, and consequently real duplicate reprocessing of already-completed documents (RabbitMQ redelivers an un-acked message, but the file was already deleted by the first, successful pass, so the redelivery fails with `"File not found"`). Dropping `MAX_CONCURRENT_MESSAGES` from 3 to 1 reduced but did **not** eliminate this — confirmed by direct reproduction: the race is between the one worker thread and the main I/O thread, not between multiple workers, so concurrency count was never the actual variable.

**Fix 1:** `connection.add_callback_threadsafe()` — added `ack_message()`/`nack_message()` helper functions and schedule them via `add_callback_threadsafe(functools.partial(...))` from `process_message()`, instead of calling `channel.basic_ack()`/`basic_nack()` directly from the worker thread. This runs the actual ack/nack on the connection's own I/O thread, which is the documented-safe pattern for `pika`'s `BlockingConnection` + threading. The plain `channel_lock` is removed entirely (no longer needed — `callback()` itself already runs on the I/O thread, so its own nack-on-queue-error path was always safe without it).

**Problem 2 — unsynchronized embedding-model lazy singleton:** `store.py`'s `_get_embedding_model()` used a bare `if _embedding_model is None: _embedding_model = SentenceTransformer(...)` with no lock. With `MAX_CONCURRENT_MESSAGES=2`, two worker threads processing two different documents could both observe `None` and race to construct the model concurrently, corrupting PyTorch's device-placement/lazy-loading state — confirmed with a real error: `RuntimeError: Cannot copy out of meta tensor; no data! Please use torch.nn.Module.to_empty() instead of torch.nn.Module.to() when moving module from meta to a different device.`

**Fix 2:** double-checked locking — `_embedding_model_lock = threading.Lock()`, only acquired when the model looks unset, re-checked inside the lock before constructing. Keeps the common case (model already loaded) lock-free.

**Verification:** all 69 automated tests still pass. Real reproduction: with both fixes deployed, retried every previously-failed document — all succeeded cleanly with zero recurrence of `"Channel is closed"`, `"File not found"`, or the meta-tensor error. Consumer pod restart count stayed at 0 throughout the retry (separately confirms Problem 1/2's fixes didn't just mask symptoms via a crash-and-recover cycle). Also separately found and fixed during the same session: Pinecone's best-effort dual-write silently fell behind ChromaDB during the period these bugs were active (intermittent `"Failed to connect; did you specify the correct index name?"` errors, unrelated to the two bugs above - a transient Pinecone-side connection issue). Backfilled via `add_chunks()` re-run from ChromaDB's existing chunks (idempotent, chunk_id-keyed upsert) until Pinecone's vector count matched ChromaDB's; final real end-to-end query in cloud mode correctly retrieved and cited chunks from multiple of the previously-affected documents.

**Note:** `MAX_CONCURRENT_MESSAGES` was left at 2 (not restored to 3) — a separate, real OOMKill was also observed at 3 concurrent workers against this pod's 1.5Gi memory limit (`kubectl describe pod` showed `OOMKilled`, `Restart Count` incrementing). That's a resource-limit issue, not fixed by either change above; raising the memory limit would be required before increasing concurrency again.
